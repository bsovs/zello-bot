...
const zello_url = 'https://discordapp.com/channels/766804341439856670/766804341439856673/775445505476001792';

class Direct {
    table(message, description, thumbnail, accounts, isToUser) {
        const getFields = (list) => {
            if (typeof list !== 'object' || list == null) return list;
            return list.map(item => {
                return ({'name': item.name, 'value': item.value, 'inline': true})
            })
        };
        const bankEmbed = new Discord.MessageEmbed()
            ...
            .setThumbnail(thumbnail != null ? thumbnail : zello_url)
    ...
    embed(message, title, description, thumbnail, footer, isToUser) {
